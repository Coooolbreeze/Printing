...
class AccumulatePointsRecord extends Model
{
     ...
     * @throws \Throwable
     */
    public static function income($number, $describe)
    {
        \DB::transaction(function () use ($number, $describe) {
            TokenFactory::getCurrentUser()->increment('accumulate_points', $number);
            TokenFactory::getCurrentUser()->increment('history_accumulate_points', $number);

            self::create([
                'user_id' => TokenFactory::getCurrentUID(),
                'number' => $number,
                'surplus' => TokenFactory::getCurrentUser()->accumulate_points,
                'describe' => $describe,
                'type' => 1
            ]);
        });
     ...
     * @throws \Throwable
     */
    public static function expend($number, $describe)
    {
        if (TokenFactory::getCurrentUser()->accumulate_points < $number)
            throw new BaseException('可用积分不足');

        \DB::transaction(function () use ($number, $describe) {
            TokenFactory::getCurrentUser()->decrement('accumulate_points', $number);
